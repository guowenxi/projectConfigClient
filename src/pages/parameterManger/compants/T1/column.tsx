...
const columns = {
    ...
    return [
      {
        ...
        render: (text: any, record: any) => {
          return (
            isedit == 1 && record.flexd != 1 ? <div><Input
              maxLength={25}
              defaultValue={text}
              onInput={(e: any) => {
                record.stateName = e.currentTarget.value
              }} /></div> : <div>{text}</div>
          )
        }
      },
  ...
  otherTableColumns2: (isedit: any, setfn?: any) => {
    return [
      {
        title: '输入值',
        dataIndex: 'name',
        ...
        render: (text: any, record: any) => {
          return (
            isedit == 1 ? <div><Input onInput={(e: any) => {
              record.name = e.currentTarget.value
            }} /></div> : <div>{text}</div>
          )
        }
      },
      {
        ...
        render: (text: any, record: any) => {
          return (
            isedit == 1 ? <div><Input /></div> : <div>{text}</div>
          )
        }
      },
    ];
