...
interface timeChooseType {
  ...
  TimeSend: any // 外部的时间的数据
  setTimeSend: any //  set useState 参数 用来给父组件传递参数
}
const template: React.FC<any> = (props: any) => {
  ...
  const [delList, set_delList] = useState<any>([]);
  ...
  useEffect(() => {
    if (edit) {
      if (!!TimeSend[setTimeIdx] && TimeSend[setTimeIdx].length != 0) {
        settimeList([...TimeSend[setTimeIdx]])
      }
    }
  }, [TimeSend])
  ...
  const changeNum = (type: string, idxNo?: any) => {
    ...
    if (type == 'add') {
      ...
      if (!!newTimeList[idxNo]?.id) {
        set_delList([...delList, newTimeList[idxNo]?.id])
      }
  ...
  return (
    <>
      {
        timeList.map((res: any, idx: any) => {
          return <div style={{ marginBottom: '10px' }}>
            <DatePicker
              picker={'time'}
              format={'HH:mm'}
              value={(res.startTime != '' && res.startTime != null) ? moment(res.startTime, 'YYYY-MM-DD HH:mm:ss') : ''}
              onChange={(e) => {
                if (e == null) {
                  timeChange('start', null, idx)
                } else {
                  timeChange('start', moment(e).format('YYYY-MM-DD HH:mm:ss'), idx)
                }
                // moment(e).format('LTS')  
              }} />
            <span style={{ margin: '0px 5px', color: 'black' }}> ——</span>
            <DatePicker
              picker={'time'}
              format={'HH:mm'}
              value={(res.endTime != '' && res.endTime != null) ? moment(res.endTime, 'YYYY-MM-DD HH:mm:ss') : ''}
              onChange={(e) => {
                if (e == null) {
                  timeChange('end', null, idx)
                } else {
                  timeChange('end', moment(e).format('YYYY-MM-DD HH:mm:ss'), idx)
                }
              }} />
            <span onClick={() => {
              changeNum('add')
